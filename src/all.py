...
class FileSelectorApp(ctk.CTk):
    def __init__(self, directory):
        ...
        logging.debug("Initializing FileSelectorApp with directory: %s", self.directory)
    ...
    def populate_files(self):
        logging.debug("Populating files for directory: %s", self.directory)
        self.clear_file_list()  # Clear any existing file checkboxes

        for root, dirs, files in os.walk(self.directory):
            logging.debug("Walking through directory: %s", root)

            # Skip the node_modules, .next, and .git directories
            if 'node_modules' in dirs:
                logging.debug("Skipping node_modules directory in: %s", root)
                dirs.remove('node_modules')
            if '.next' in dirs:
                logging.debug("Skipping .next directory in: %s", root)
                dirs.remove('.next')
            if '.git' in dirs:
                logging.debug("Skipping .git directory in: %s", root)
                dirs.remove('.git')
            ...
            if rel_root == ".":
                ...
                if rel_root not in self.folder_vars:
                    self.folder_vars[rel_root] = ctk.StringVar(value="1")  # Mark folders by default
            ...
            for dir_name in dirs:
                logging.debug("Processing directory: %s", dir_name)
...
if __name__ == "__main__":
    ...
    logging.debug("Starting FileSelectorApp with directory: %s", directory)
